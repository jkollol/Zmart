...
<div class="min-h-screen bg-gradient-to-br from-green-100 via-green-50 to-white flex items-center justify-center p-6">
    <div class="max-w-7xl w-full bg-white rounded-2xl shadow-2xl p-10 mx-auto">
        <header class="mb-10 text-center">
            <h1 class="text-5xl font-extrabold text-green-700 mb-3">Dashboard</h1>
            <p class="text-green-600 text-lg">Welcome back, <span class="font-semibold">{{ auth()->user()->name }}</span>!</p>
        </header>

        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            <!-- Profile Card -->
            <a href="{{ route('profile.edit') }}"
               class="flex flex-col justify-center items-start bg-green-100 rounded-xl shadow-md p-6 hover:bg-green-200 transition-colors duration-300 group">
                <div class="mb-4">
                    <svg class="w-10 h-10 text-green-700 group-hover:text-green-900 transition-colors" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" >
            ...
            <a href="{{ route('checkout') }}"
               class="flex flex-col justify-center items-start bg-green-100 rounded-xl shadow-md p-6 hover:bg-green-200 transition-colors duration-300 group">
                <div class="mb-4">
                    <svg class="w-10 h-10 text-green-700 group-hover:text-green-900 transition-colors duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <form method="POST" action="{{ route('logout') }}"
                  class="flex flex-col justify-center items-start bg-red-100 rounded-xl shadow-md p-6 hover:bg-red-200 transition-colors duration-300 group">
                @csrf
                <button type="submit" class="w-full text-left">
                    <div class="mb-4">
                        <svg class="w-10 h-10 text-red-700 group-hover:text-red-900 transition-colors duration-300" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
